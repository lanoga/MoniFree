...
<template>
  <div class="relative">
    <a ref="trigger" @click.stop="toggleDropdown" class="flex items-center gap-4" href="#">
      <span class="hidden text-right lg:block">
        <span class="block text-sm font-medium text-black dark:text-white">{{
          `${auth.userData?.lastName} ${auth.userData?.firstName}`
        }}</span>
        <span class="block text-xs">{{ auth.userData?.role }}</span>
      </span>

      <span class="h-12 w-12 rounded-full">
        <img src="@/assets/images/user.png" :alt="auth.userData?.name" />
      </span>
    ...
    >
      <ul class="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark"></ul>
      <button
        @click="logout"
